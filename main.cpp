...
PubSubClient mqttClient(IOT_ENDPOINT, 8883, msgReceived, secureClient);
IOT iotclient(secureClient, mqttClient);
bool subscribed = false;
bool accident_data_available = false;
...
void setup()
{
    ...
    subscribed = mqttClient.subscribe("users/sos/image", 0);
    if (subscribed){
...
void pubSubCheckConnect() {
  if ( ! mqttClient.connected()) {
    ...
    while ( ! mqttClient.connected()) {
      Serial.print(".");
      mqttClient.connect("ESPthingXXXX");
      delay(1000);
    }
    Serial.println(" connected");
    mqttClient.subscribe("users/sos/image", 0);
  }
  mqttClient.loop();
}
...
{
    pubSubCheckConnect();
    if (get_data() && accident_data_available)
    {
        if (iotclient.publish("users/images", fb->buf, fb->len))
        {
